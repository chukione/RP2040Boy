...
#endif
#ifdef USE_PICO
  pinMode(pinGBClock, INPUT_PULLDOWN);
#else
  ...
  while (1) {
    ...
    if (serial->available()) { // If serial data was send to midi input
      ...
      if (!checkForProgrammerSysex(incomingMidiByte) && !usbMode)
        serial->write(incomingMidiByte); // Send it to the midi output
    }
    ...
    if (readgbClockLine) {                         // If Gb's Clock is On
      while (readgbClockLine) {                    // Loop untill its off
        ...
        bit = digitalRead(
            pinGBSerialIn); // Read the serial input for song position
        checkActions();
      }

      countClockPause = 0; // Reset our wait timer for detecting a sequencer
                           // stop

      readGbSerialIn = readGbSerialIn << 1; // left shift the serial byte by one
                                            // to append new bit from last loop
      readGbSerialIn =
          readGbSerialIn + bit; // and then add the bit that was read

      sendMidiClockSlaveFromLSDJ(); // send the clock & start offset data to
                                    // midi
    }
    setMode();
  }
...
/*
 checkLSDJStopped counts how long the clock was on, if its been on too long we
 assume LSDJ has stopped- Send a MIDI transport stop message and return true.
*/
...
void sendMidiClockSlaveFromLSDJ() {
  if (!countGbClockTicks) {  // If we hit 8 bits
    if (!sequencerStarted) { // If the sequencer hasnt started
      serial->write((
          0x90 + memory[MEM_LSDJMASTER_MIDI_CH])); // Send the midi channel byte
...
#ifdef USE_TEENSY
      usbMIDI.sendNoteOn(memory[MEM_LSDJMASTER_MIDI_CH] + 1, readGbSerialIn,
                         0x7F);
...
#endif
#ifdef USE_LEONARDO
      midiEventPacket_t event = {0x09,
                                 0x90 | memory[MEM_LSDJMASTER_MIDI_CH] + 1,
                                 readGbSerialIn, 0x7F};
      MidiUSB.sendMIDI(event);
      MidiUSB.flush();
      event = {0x0F, 0xFA};
